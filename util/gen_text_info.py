...
def process_WSSS4LUAD_binary(root_dir, seed, train_ratio):
    ...
    for file in tqdm(os.listdir(path2data)):
        ...
        class_ = np.array(file.split('[')[1].split(']')[0].split(', ')).astype(
            int)  # Multi-class labels: [Tumor, Stroma, Normal]
        if class_[0] == 1:
            lbl = 1  # has tumor
        else:
            lbl = 0  # no tumor
...
def process_DigestPath(root_dir, seed=None, train_ratio=None):
    ...
    df_neg['image'] = [opj(dd, 'images', 'negative', '%05d.png' % (i)) for i, (filename, downsample) in
                       enumerate(zip(final_negative_stats['filename'], final_negative_stats['downsample']))]
    df_pos['image'] = [opj(dd, 'images', 'positive', '%05d.png' % (i)) for i, (filename, downsample) in
                       enumerate(zip(final_positive_stats['filename'], final_positive_stats['downsample']))]
    df_neg['label'] = 0
...
def process_PanNuke(root_dir, seed=None, train_ratio=None):
    df = pd.read_csv(
        '/home/liuq23/data/image-text_colearning/PLIP/data/PanNuke/processed_threshold=10_0.3/PanNuke_all_binary.csv',
        index_col=0)
    df = df.reset_index(drop=True)
    for i in df.index:
